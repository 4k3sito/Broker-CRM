...
def status(out_path, states=None, stall_after: float = 600.0, _pids=None) -> int:
    ...
    sleeping blind — **0** healthy (running, or finished with every query
        ...
        until .venv/bin/python viva_scraper.py --status; do sleep 300; done
    """
    ...
    urls = load_sitemap()
    planned = [f"{root}/{loc}" for root, _ in SEARCHES
               for loc in sorted(shard_tree(urls, root)[""])
               if not states or loc in states]
...
def main() -> None:
    ...
    if args.status:
        sys.exit(status(args.out, set(args.states) if args.states else None))
    elif args.selfcheck:
        _selfcheck()
    elif args.audit:
        audit(args.audit)
